...
設備: ${extracted.facilities && extracted.facilities.length > 0 ? extracted.facilities.join("、") : "（記載なし）"}
担当者メモ（おすすめポイント）: ${extracted.recommendPoint?.trim() ? extracted.recommendPoint.trim() : "（なし）"}
...
【コメントの構成（3〜4文）】
1文目：この物件の一番のセールスポイント。上の「セールスポイント」に項目がある場合は必ずそれを最優先で具体的に書く。特にインターネット無料のような毎月の支出が減る特徴は、金銭メリットが伝わるトーンで書く（例：「インターネット無料なので、毎月の通信費がかかりません」）。セールスポイントが無ければ間取り・面積・立地のいずれか1つを具体的に書く
...
- セールスポイントや設備は、上の情報に書いてあるものだけ触れる。書いていない特徴（無料・割引・設備など）を勝手に作らない
